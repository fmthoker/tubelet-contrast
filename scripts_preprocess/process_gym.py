...
def parse_args():
    ...
    parser.add_argument('--raw_dir', default='/ssd/fmthoker/gym/subactions/',
                        type=str, help='raw data directory')
    parser.add_argument('--out_dir', default='/ssd/fmthoker/gym/videos_ctp/',
                        type=str, help='output data directory.')
    parser.add_argument('--ann_dir', default='/ssd/fmthoker/gym/annotations/',
...
if __name__ == '__main__':
    ...
    for prefix in ['train', 'val']:
        ann_file = os.path.join(args.ann_dir, f'gym99_{prefix}.txt')
        video_name_list = [sp[0][:-4] for sp in load_file(ann_file)]
        video_label_list = [int(sp[1]) for sp in load_file(ann_file)]

        out_file = os.path.join(args.out_dir, 'annotations', f'gym_99_{prefix}.txt')
